...
module rom16_8 (
  ...
  initial begin
    $readmemh("mem.mem", ROM);      // поместить в память RAM содержимое
  end                               // файла mem.mem
...
Содержимое файла `mem.mem`, к примеру может быть таким (каждая строка соответствует значению отдельной ячейки памяти, начиная со стартового адреса):
